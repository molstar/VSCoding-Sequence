...
function addHelices(cat, coordinates, map, elements) {
    ...
    for (let i = 0, _i = cat._rowCount; i < _i; i++) {
        const type = types_1.SecondaryStructureType.create(pdbx_PDB_helix_class.valueKind(i) === 0 /* Column.ValueKinds.Present */
...
function addSheets(cat, coordinates, map, sheetCount, elements) {
    ...
    for (let i = 0, _i = cat._rowCount; i < _i; i++) {
        ...
        const type = types_1.SecondaryStructureType.create(4 /* SecondaryStructureType.Flag.Beta */ | 8388608 /* SecondaryStructureType.Flag.BetaSheet */);
        const element = {
            kind: 'sheet',
            flags: type,
            sheet_id: id,
