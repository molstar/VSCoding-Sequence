...
const boundary_2 = require("./util/boundary");
class Structure {
...
function getSerialMapping(structure) {
    ...
    return {
        ...
        unitIndices,
        elementIndices,
        getSerialIndex: (unit, element) => cumulativeUnitElementCount[unitIndexMap.get(unit.id)] + int_1.OrderedSet.indexOf(unit.elements, element)
    };
}
(function (Structure) {
    ...
    function ofModel(model, props = {}) {
        ...
        for (let c = 0; c < chains.count; c++) {
            ...
            let singleAtomResidues = atomic_1.AtomicHierarchy.chainResidueCount(model.atomicHierarchy, c) === chains.offsets[c + 1] - chains.offsets[c];
    ...
    function partitionAtomicUnitByAtom(model, indices, builder, multiChain, operator) {
        ...
        const lookup = (0, geometry_1.GridLookup3D)(position, (0, boundary_1.getBoundary)(position), 8192);
        const { offset, count, array } = lookup.buckets;
        const traits = (multiChain ? unit_1.Unit.Trait.MultiChain : unit_1.Unit.Trait.None) | (offset.length > 1 ? unit_1.Unit.Trait.Partitioned : unit_1.Unit.Trait.None);
    ...
    function partitionAtomicUnitByResidue(model, indices, builder, multiChain, operator) {
        ...
        const startIndices = [];
        const endIndices = [];
        const residueIt = int_1.Segmentation.transientSegments(residueAtomSegments, indices);
        ...
        const { x, y, z } = model.atomicConformation;
        const position = { x, y, z, indices: int_1.SortedArray.ofSortedArray(startIndices) };
        const lookup = (0, geometry_1.GridLookup3D)(position, (0, boundary_1.getBoundary)(position), gridCellCount);
        const { offset, count, array } = lookup.buckets;
        const traits = (multiChain ? unit_1.Unit.Trait.MultiChain : unit_1.Unit.Trait.None) | (offset.length > 1 ? unit_1.Unit.Trait.Partitioned : unit_1.Unit.Trait.None);
        builder.beginChainGroup();
        for (let i = 0, _i = offset.length; i < _i; i++) {
