...
const AssemblySymmetryVisuals = {
    ...
    'cage': (ctx, getParams) => ShapeRepresentation(getCageShape, Mesh.Utils, { modifyState: s => ({ ...s, markerActions: MarkerActions.Highlighting }) }),
    'axes': (ctx, getParams) => ShapeRepresentation(getAxesShape, Mesh.Utils, { modifyState: s => ({ ...s, markerActions: MarkerActions.Highlighting }) }),
};
...
const tmpScale = Vec3();
const getOrderPrimitive = memoize1((order) => {
    if (order < 2) {
...
function getAxesMesh(data, props, mesh) {
    ...
    if (!AssemblySymmetryData.isRotationAxes(rotation_axes))
...
//
const getSymbolCage = memoize1((symbol) => {
