...
export class VolsegLatticeSegmentationData {
    ...
    async loadSegmentation() {
        ...
        if (hasLattices) {
            ...
            if (!group) {
                const newGroupNode = await this.entryData.newUpdate().apply(CreateGroup, { label: 'Segmentation', description: 'Lattice' }, { tags: [GROUP_TAG], state: { isCollapsed: true } }).commit();
