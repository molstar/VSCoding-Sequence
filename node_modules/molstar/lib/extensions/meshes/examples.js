...
export async function runMeshExtensionExamples(plugin, db_url = DB_URL) {
...
export async function runMeshExample(plugin, segments, db_url = DB_URL) {
    const detail = 2;
...
/** Example for downloading multiple separate segments, each containing 1 mesh. */
export async function runMeshExample2(plugin, segments) {
    const detail = 1;
...
export async function runMultimeshExample(plugin, segments, detailChoice, db_url = DB_URL) {
...
export async function runMeshStreamingExample(plugin, source = 'empiar', entryId = 'empiar-10070', serverUrl, parent) {
    const params = ParamDefinition.getDefaultValues(MeshServerInfo.Params);
    if (serverUrl)
        params.serverUrl = serverUrl;
    params.source = source;
    params.entryId = entryId;
    await plugin.runTask(plugin.state.data.applyAction(InitMeshStreaming, params, parent === null || parent === void 0 ? void 0 : parent.ref), { useOverlay: false });
}
/** Example for downloading a protein structure and visualizing molecular surface. */
export async function runMolsurfaceExample(plugin) {
...
export async function runIsosurfaceExample(plugin, db_url = DB_URL) {
    var _a, _b;
