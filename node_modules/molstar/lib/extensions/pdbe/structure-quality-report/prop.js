...
export const StructureQualityReportProvider = CustomModelProperty.createProvider({
    ...
    descriptor: CustomPropertyDescriptor({
        ...
        cifExport: {
            ...
            categories: [
                PropertyWrapper.defaultInfoCategory('pdbe_structure_quality_report', ctx => ctx.info),
                {
        ...
        symbols: {
            issueCount: QuerySymbolRuntime.Dynamic(CustomPropSymbol('pdbe', 'structure-quality.issue-count', Type.Num), ctx => StructureQualityReport.getIssues(ctx.element).length),
            // TODO: add (hasIssue :: IssueType(extends string) -> boolean) symbol
        }
    }),
    type: 'static',
    defaultParams: StructureQualityReportParams,
    getParams: (data) => StructureQualityReportParams,
...
function createExportContext(ctx) {
    ...
    let info = PropertyWrapper.createInfo();
