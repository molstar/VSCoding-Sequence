...
export class G3dHeaderObject extends SO.Create({ name: 'G3D Header', typeClass: 'Data' }) {
}
...
export const G3DTrajectory = PluginStateTransform.BuiltIn({
    ...
    params: a => {
        ...
        return {
            resolution: ParamDefinition.Select(rs[rs.length - 1], rs.map(r => [r, '' + r]))
        };
    },
...
export const LoadG3D = StateAction.build({
    ...
    from: SO.Root,
    params: { url: ParamDefinition.Text('') }
})(({ params, state }, ctx) => Task.create('Genome3D', taskCtx => {
