...
const _ScreenshotPreview = (props) => {
    ...
    return _jsx(_Fragment, { children: _jsxs("div", { style: { position: 'relative', width: '100%', height: '100%' }, children: [_jsx("canvas", { ref: canvasRef, onContextMenu: e => { e.preventDefault(); e.stopPropagation(); }, style: { display: 'block', width: '100%', height: '100%' } }), _jsx(ViewportFrame, { plugin: plugin, canvas: currentCanvas, color: cropFrameColor })] }) });
};
...
function ViewportFrame({ plugin, canvas, color = 'rgba(255, 87, 45, 0.75)' }) {
    ...
    const cropFrameRef = useRef({ x: 0, y: 0, width: 0, height: 0 });
