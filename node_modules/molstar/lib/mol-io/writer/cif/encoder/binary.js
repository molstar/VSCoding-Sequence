...
function getFieldData(field, arrayCtor, totalCount, data) {
    ...
    for (var _d = 0; _d < data.length; _d++) {
        ...
        while (keys.hasNext) {
            var key = keys.move();
            var p = valueKind ? valueKind(key, d) : 0 /* Column.ValueKind.Present */;
            if (p !== 0 /* Column.ValueKind.Present */) {
                mask[offset] = p;
                if (isStr)
                    array[offset] = '';
                allPresent = false;
                ...
                if (typeof value === 'string' && !value) {
                    mask[offset] = 1 /* Column.ValueKind.NotPresent */;
                    allPresent = false;
                }
                else {
                    mask[offset] = 0 /* Column.ValueKind.Present */;
                }
                array[offset] = value;
            }
            offset++;
